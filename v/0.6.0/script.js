const STORAGE_KEY = "yt_playlist_tracker_state_v7";
const EXPORT_VERSION = "0.6.0";
...
function mergeVideos(incomingVideos, mode) {
    const incoming = incomingVideos.map((video, index) => ({
        ...
        manualOrder: Number(video.manualOrder || video.baseOrder || index + 1)
    }));

    if (mode === "replace") {
        state.videos = incoming.map((video, index) => ({
            ...video,
            baseOrder: index + 1,
            manualOrder: index + 1
        }));
    ...
    if (mode === "top") {
        ...
        const manualOrdered = [...incomingUnique, ...existingFiltered.slice().sort((a, b) => (a.manualOrder || 0) - (b.manualOrder || 0))];
        const manualOrderMap = new Map(manualOrdered.map((video, index) => [video.id, index + 1]));

        state.videos = baseOrdered.map((video, index) => ({
            ...video,
            baseOrder: index + 1,
            manualOrder: manualOrderMap.get(video.id) || index + 1
        }));
    } else {
