const STORAGE_KEY = "yt_playlist_tracker_state_v3";
...
async function prefillAndPrefetchFromQuery() {
    ...
    await fetchVideosFlow({ forceReplace: true, silentPrompt: true });
}
...
async function fetchVideosFlow(options = {}) {
    ...
    if (state.videos.length > 0 && !options.silentPrompt) {
        shouldReplace = window.confirm("Replace existing list? Click Cancel to add fetched video(s) with current list.");
    }
    ...
    try {
        ...
        if (mixed.playlistIds.length > 0) {
            ...
            incomingVideos.push(
                ...uniquePlaylistItems.map((item, index) => createVideoModel(item, playlistDetailMap, index))
            );
        }
        ...
        mergeVideos(uniqueIncoming, shouldReplace || options.forceReplace === true);
...
function mergeVideos(incomingVideos, replaceExisting) {
    if (replaceExisting) {
...
function computeFinishEta(remainingSeconds, speed, bufferMinutes) {
    ...
    const base = getStartBaseDate();
    const finishDate = new Date(base.getTime() + adjustedSeconds * 1000);
