const STORAGE_KEY = "yt_playlist_tracker_state_v5";
...
const refs = {
    ...
    includeApiKey: document.getElementById("includeApiKey"),
...
function mergeVideos(incomingVideos, mode) {
    ...
    if (mode === "top") {
        ...
        state.videos = [...incomingUnique.map((video) => ({ ...video })), ...existingFiltered];
    } else {
...
function exportData() {
    const includeApiKey = refs.includeApiKey.checked;
    const now = new Date();
